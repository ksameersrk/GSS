...
/**
 * Created by spadigi on 2/9/17.
 */
public class SpinningDownAlgo1 {
    public static void main(String[] args) throws Exception {
        ...
        String inputLog = "files/basic/SpinningDownAlgo1/inputIdealDataset100Ops.txt";
        ...
        // send nodeToTask for analysing how many disks can be spun down
        System.out.println("\n\n Total no. of HDDs : " + totalNoOfNodes);
        System.out.println("\n\n No. of disks that could ne spun down are : " + getNoOfDisksThatCouldBeSpunDown(nodeToTaskMapping));
    }
    ...
    public static int getNoOfDisksThatCouldBeSpunDown(HashMap<Node, Tasks> nodeToTaskMapping)
    {
        HashMap<Node, ArrayList<String>> nodeToFileMapping = new HashMap<Node, ArrayList<String>>();
        ArrayList<Node> allNodes = new ArrayList<Node>(nodeToTaskMapping.keySet());
        allNodes.sort(Comparator.comparing(Node::getID));
        for (Node n : allNodes) {
            String tasks = nodeToTaskMapping.get(n).getFiles();
            tasks = tasks.replaceAll("\\s+","");
            ArrayList<String> files = new ArrayList<String>(new HashSet<String>(Arrays.asList(tasks.split(","))));
            nodeToFileMapping.put(n, files);
        }
